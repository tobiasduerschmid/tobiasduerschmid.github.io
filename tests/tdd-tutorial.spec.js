...
const A11Y_FEATURE = 'tdd-tutorial';
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (step.tests?.length > 0) {
      test(`step ${i + 1} "${step.title}": solution passes all ${step.tests.length} tests`, async () => {
        ...
        await a11yCheckpoint(page, `tdd tutorial — step ${i + 1} all tests passing`, { feature: A11Y_FEATURE, darkMode: true });
      });
    }
    ...
    if (step.quiz?.questions?.length > 0 && !isLast) {
      test(`step ${i + 1} "${step.title}": quiz gate — advances to step ${i + 2}`, async () => {
        ...
        await a11yCheckpoint(page, `tdd tutorial — step ${i + 1} quiz gate (first question)`, { feature: A11Y_FEATURE, darkMode: true });
        ...
        await a11yCheckpoint(page, `tdd tutorial — step ${i + 1} quiz results`, { feature: A11Y_FEATURE, darkMode: true });
