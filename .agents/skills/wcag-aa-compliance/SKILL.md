...
description: Project rule that every reachable page must remain fully compliant with WCAG 2.2 Level AA. USE THIS SKILL EVERY SINGLE TIME you write, edit, review, or remove anything that ends up in the user-facing surface — any HTML / Liquid / Jekyll include / layout / page (`*.html`, `_includes/*`, `_layouts/*`), any CSS / SCSS / inline `<style>` / JS-injected stylesheet, any JavaScript that creates / modifies / shows / hides / focuses / labels DOM elements, any SVG (icons, diagrams, charts, decorative graphics), any image (`<img>`, `background-image`, `<picture>`, `<source>`), any `<video>` / `<audio>` / `<track>`, any form control (`<input>`, `<select>`, `<textarea>`, `<button>`, custom widgets, contenteditable surfaces), any color / background / border / shadow / outline change, any focus / hover / active / disabled state, any modal / dialog / popover / tooltip / popout window, any toast / live region / status message / progress indicator, any keyboard handler / shortcut / chord, any drag / pointer / touch / motion handler, any animation / transition / scroll-snap / parallax / autoplay effect, any navigation / menu / breadcrumb / skip-link, any heading / landmark / list / table, any iframe / embedded widget (quizzes, flashcards, code editors, terminals, UML / Mermaid / ArchUML diagrams, audio players), any print stylesheet, and the structural / embedded elements (images with alt text, links with link text, embedded HTML, inline `<style>`, Liquid includes, ArchUML / Mermaid code blocks, tables, headings whose level is changing) inside a Markdown file. Trigger when adding new components / widgets / pages / layouts, changing existing visuals or interactions, fixing bugs that touch the rendered page, refactoring shared CSS, bumping a frontend dependency that emits markup or styles, or merging anything that produces output the browser displays. Also trigger on requests like "make this look nicer", "add a button / modal / banner", "fix this color", "add a tooltip", "the page looks weird at small widths", "add a video / audio clip", "add a chart / diagram", "let users drag this around", or any prompt where the change is observable in the browser. **DO NOT trigger this skill for pure-prose edits to Markdown content (`*.md`, including SEBook chapters, tutorial Markdown, and blog posts) — typo fixes, sentence rewordings, paragraph rewrites, list-item text changes, and code-block content changes that only alter the words inside existing paragraphs / list items / code blocks without adding, removing, restructuring, or renaming any image, link, heading, table, embedded HTML, inline `<style>`, Liquid include, SVG, or embedded widget.** Every triggering change must leave the site compliant with all 55 Level A and Level AA success criteria of WCAG 2.2; the audit test at `tests/wcag22-complete-audit.spec.js` enforces this in CI. If you cannot make a change AA-compliant, stop and surface the trade-off to the user before proceeding — this is non-negotiable.
---
...
The project rule is simple and absolute: **every reachable page must satisfy [WCAG 2.2 Level AA](https://www.w3.org/TR/WCAG22/) at all times.** That means meeting all 30 Level A criteria *and* all 25 Level AA criteria — 55 success criteria in total — on every page the build emits. This applies to new features, bug fixes, refactors, structural / embedded content changes (images, links, headings, tables, embedded HTML, Liquid includes, ArchUML / Mermaid blocks, SVG, inline `<style>`), and dependency bumps. There is no "we'll fix it later" grace period; if you cannot make your change AA-compliant, stop and surface the trade-off to the user before proceeding.

**Scope exclusion — pure-prose Markdown edits.** This skill does **not** apply to pure-prose edits inside Markdown files (`.md`) — typo fixes, sentence rewordings, paragraph rewrites, list-item text changes, and code-block content changes that only alter the words inside existing paragraphs / list items / code blocks. The Markdown→HTML pipeline preserves the surrounding structure, so prose-only edits cannot regress any of the 55 criteria. The skill *does* re-engage the moment a Markdown edit adds, removes, renames, or restructures an image, link, heading level, table, embedded HTML, inline `<style>`, Liquid include, SVG, or embedded widget — those affect 1.1.1 (alt text), 2.4.4 (link purpose), 1.3.1 / 2.4.6 (headings), 1.3.1 (table semantics), 1.4.3 (contrast), and other criteria that the Markdown body alone cannot guarantee.
...
- **Styling `a` color without pairing `:visited`** — every CSS rule that sets `color` (or `background-color`) on an anchor must include `:visited` in the same comma-separated selector list. Visited and unvisited links must look identical on this site (1.4.1, 1.4.3): never let the browser's default purple — or a stale "visited" fade — slip in. The robust pattern is `.foo a, .foo a:visited { color: #2774AE; }` (don't rely on the global `a, a:visited` rule to "carry through" — a more-specific rule that lacks `:visited` will tie on specificity and the source order may flip the visited color in subtle ways). State-only rules (`:hover`, `:focus`, `:active`, `:focus-visible`) are exempt — they apply on top of both link and visited. Enforced by [`tests/visited-link-parity.spec.js`](../../../tests/visited-link-parity.spec.js).
