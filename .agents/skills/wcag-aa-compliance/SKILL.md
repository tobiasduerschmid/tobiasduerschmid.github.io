...
description: Project rule that every reachable page must remain WCAG 2.2 Level AA compliant. Use when editing browser-facing HTML, Liquid, Markdown structure, CSS, JavaScript DOM behavior, SVG, images, media, forms, controls, colors, focus states, modals, tooltips, status messages, keyboard or pointer interactions, animations, navigation, headings, tables, iframes, embedded widgets, print styles, or frontend dependencies. Do not use for pure data/text edits that cannot affect accessibility, such as difficulty metadata, non-rendered tags or IDs, typo fixes, or wording changes inside existing prose/code blocks without structural or semantic changes. Triggering changes require appropriate accessibility verification; non-impact data/text changes should use lightweight data/build checks instead of WCAG audits.
---
