...
The mental challenge you feel when you have to manually track down a missing bracket or debug a loop is exactly the kind of effortful struggle that ensures you will be able to catch bugs more quickly tomorrow.
...
*   **Computer Science Example:** When you are starting to write a complex loop or pointer arithmetic, try to recall the syntax from memory instead of looking it up. When you are learning version control with `git`, instead of looking up the documentation of each command before you use it, try to recall it from memory. When you are writing a sort function, try to remember all sorting algorithms you have learned and how they are differnet. **At every possible opportunity, try to use your own memory before consulting other ressources.**
